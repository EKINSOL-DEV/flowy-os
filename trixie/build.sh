...
set -euo pipefail
export COPYFILE_DISABLE=1  # geen macOS ._AppleDouble in tar/cp
...
# DEV-ONLY: bake a wifi connection so the fresh image is reachable over SSH.
# Local, gitignored file. Remove from the device once provisioning is done.
WIFI_KEYFILE="${FLOWY_WIFI_KEYFILE:-$HOME/Downloads/flowy-trixie/EKINSOL-DEVICES.nmconnection}"
if [ -f "$WIFI_KEYFILE" ]; then
  mkdir -p "$STAGE/overlay/etc/NetworkManager/system-connections"
  cp "$WIFI_KEYFILE" "$STAGE/overlay/etc/NetworkManager/system-connections/"
  echo "  (wifi keyfile baked in — dev only)"
fi
...
log "oude build-chroot opruimen (namespace-root pyc's blokkeren anders hergebruik)"
"${SSH[@]}" "cd ~/rpi-image-gen && echo ${PASS} | sudo -S rm -rf work/chroot-* work/bootstrap work/build work/deploy-* work/image-flowy-trixie 2>/dev/null; true"
